...
def draw_background():
    """Draw a garden-like grassy background."""
    for x in range(0, WINDOW_WIDTH, CELL_SIZE):
        for y in range(0, WINDOW_HEIGHT, CELL_SIZE):
            # Random variation of green
            shade = random.randint(20, 50)
            color = (34, 139 + shade, 34) if random.random() > 0.1 else (34, 100, 34)
            pygame.draw.rect(screen, color, (x, y, CELL_SIZE, CELL_SIZE))
            # Add some brown patches for dirt
            if random.random() < 0.05:
                pygame.draw.rect(screen, LIGHT_BROWN, (x+2, y+2, CELL_SIZE-4, CELL_SIZE-4))
    # Draw subtle grass blades
    for _ in range(200):
        x = random.randint(0, WINDOW_WIDTH)
        y = random.randint(0, WINDOW_HEIGHT)
        pygame.draw.line(screen, DARK_GREEN, (x, y), (x, y-3), 1)
...
def main():
    while True:
        ...
        while running:
            ...
            current_speed = base_speed + (level - 1) * 2  # speed increases with level
